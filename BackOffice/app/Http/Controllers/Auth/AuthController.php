...
class AuthController extends Controller
{
	...
	*/
		...
		Guard $auth)
	{
        ...
        try{
				$responsJson = $this->clientAPI->request('GET', 'https://kraken-api.tpch.fr/token', 
					['auth' => [$request->input('log'), $request->input('password')]
				]);

			$JsonUsers = json_decode($responsJson->getBody(), true);
			
			if($responsJson->getStatusCode() == 200){

				session(['user' => $request->input('log')]);
